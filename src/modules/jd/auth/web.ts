...
export async function login (user) {
  ...
  // 自动填写表单
  user.username && await page.$eval('#loginname', (el, value) => (<HTMLInputElement>el).value = value, user.username)
  user.password && await page.$eval('#nloginpwd', (el, value) => (<HTMLInputElement>el).value = value, user.password)
  if (canAutoLogin) {
